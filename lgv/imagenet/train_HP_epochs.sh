...
echo
echo "Collect ResNet50 models with different epochs"
echo
...
NB_EXAMPLES=2000
BATCH_SIZE=64
#PATH_CSV="${DIR_BASE}/HP/epochs/attack.csv"
PATH_CSV="${DIR_BASE}/HP/epochs/attack_interarch.csv"
ARGS_COMMON="--n-examples $NB_EXAMPLES --shuffle --model-target-path $TARGET --data-path $DATAPATH --csv-export ${PATH_CSV} --n-models-cycle 1 --batch-size $BATCH_SIZE"
...
for i in {0..2} ; do
  ...
  for NB_EPOCHS in {1..10} ; do
    echo "------ Number of epochs: $NB_EPOCHS -----"
    NB_SAMPLES=$((NB_EPOCHS*4))
    echo "--- TEST ---"
    echo "    -- L2 attack --"
    $ATTACK $PATH_SURROGATE $ARGS_COMMON --n-iter 50 $ARGS_L2 --seed $SEED --limit-n-cycles $NB_SAMPLES
    echo "    -- Linf attack --"
    $ATTACK $PATH_SURROGATE $ARGS_COMMON --n-iter 50 $ARGS_Linf --seed $SEED --limit-n-cycles $NB_SAMPLES

    echo "--- VAL ---"
    echo "    -- L2 attack --"
    $ATTACK $PATH_SURROGATE $ARGS_COMMON --n-iter 50 $ARGS_L2 --seed $SEED --limit-n-cycles $NB_SAMPLES --validation
    echo "    -- Linf attack --"
    $ATTACK $PATH_SURROGATE $ARGS_COMMON --n-iter 50 $ARGS_Linf --seed $SEED --limit-n-cycles $NB_SAMPLES --validation
